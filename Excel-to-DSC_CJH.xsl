...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:math="http://www.w3.org/2005/xpath-functions/math" xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl" xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:x="urn:schemas-microsoft-com:office:excel" xmlns:ss="urn:schemas-microsoft-com:office:spreadsheet"
  xmlns:html="http://www.w3.org/TR/REC-html40" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:ead="urn:isbn:1-931666-22-9" xmlns:mdc="http://mdc" exclude-result-prefixes="xs math xd o x ss html xlink ead mdc" version="2.0">
  ...
  <!-- KS 20151008 added otherlevel section; tweaed unitdate -->
  ...
  <xsl:variable name="ead-copy-filename" select="ss:Workbook/ss:Worksheet[@ss:Name = 'Original-EAD']/ss:Table/ss:Row[1]/ss:Cell/ss:Data"/>
  ...
  <xsl:template match="ss:Row[ss:Cell/ss:Data]">
    ...
    <xsl:param name="following-depth" select="
    ...
    <xsl:param name="level" select="
                if (not(matches(ss:Cell[2]/ss:Data, '^(series|subseries|file|item|accession|otherlevel)$'))) then
                    'file'
                else
                    ss:Cell[2]/ss:Data/text()
                    (: in other words, if the second column of the row is blank, then 'file' will be used as the @level type by default :)" as="xs:string"/>
    ...
    <xsl:element name="{$c0x}">
      ...
      <did>
        ...
        <xsl:apply-templates select="
                        ...
                        following-sibling::ss:Row[ss:Cell[1]/ss:Data[. ne '0']]/following-sibling::ss:Row" mode="did"/>
      </did>
      ...
      <xsl:apply-templates select="
                    ...
                    following-sibling::ss:Row[ss:Cell[1]/ss:Data[. ne '0']]/following-sibling::ss:Row" mode="non-did"/>
  ...
  <xsl:template match="ss:Cell[ss:Data[normalize-space()]]" mode="did">
    ...
    <xsl:variable name="previous-index" select="xs:integer(preceding-sibling::ss:Cell[@ss:Index][1]/@ss:Index)"/>
    <xsl:variable name="cells-before-previous-index" select="count(preceding-sibling::ss:Cell[@ss:Index][1]/following-sibling::* intersect preceding-sibling::ss:Cell)"/>
    <xsl:variable name="column-number" as="xs:integer">
      <xsl:value-of select="mdc:get-column-number($position, $current-index, $previous-index, $cells-before-previous-index)"/>
    </xsl:variable>
  ...
  <xsl:template name="did-stuff">
    ...
    <xsl:choose>
      ...
      <xsl:when test="$column-number eq 22">
        ...
        <xsl:variable name="instance_type" select="preceding-sibling::ss:Cell[ss:NamedCell/@ss:Name = 'instance_type']/ss:Data"/>
        <xsl:variable name="barcode" select="preceding-sibling::ss:Cell[ss:NamedCell/@ss:Name = 'barcode']/ss:Data"/>
        ...
        <container>
          ...
          <xsl:if test="preceding-sibling::ss:Cell[ss:NamedCell/@ss:Name = 'container_profile'][ss:Data[normalize-space()]]">
            <xsl:attribute name="altrender">
              <xsl:value-of select="preceding-sibling::ss:Cell[ss:NamedCell/@ss:Name = 'container_profile']/ss:Data"/>
            </xsl:attribute>
          </xsl:if>
          <xsl:apply-templates/>
        </container>
      ...
      <xsl:when test="$column-number eq 4">
        <unittitle>
          ...
          <xsl:if test="following-sibling::ss:Cell[ss:NamedCell[@ss:Name='component_id']]/ss:Data != ''">
          ...
          select="
                        ...
                        then
                            ...
                            then
                                ...
                                ''"/>
        <xsl:variable name="month-begin" select="
                        ...
                        then
                            ...
                            ''"/>
        <xsl:variable name="day-begin" select="
          ...
          select="
                        ...
                        then
                            ...
                            then
                                ...
                                ''"/>
        <xsl:variable name="month-end" select="
                        ...
                        then
                            ...
                            ''"/>
        <xsl:variable name="day-end" select="
          ...
          <xsl:if test="$year-begin ne ''">
            <xsl:attribute name="normal">
              <xsl:choose>
                ...
                <xsl:otherwise>
                  <xsl:value-of select="concat($year-begin, $month-begin, $day-begin, '/', $year-end, $month-end, $day-end)"/>
                </xsl:otherwise>
              </xsl:choose>
            </xsl:attribute>
          </xsl:if>
      ...
      <xsl:when test="$column-number eq 12">
        <xsl:variable name="bulk-year-begin" select="
                        if (ss:Data) then
                            format-number(., '0000')
                        else
                            ''"/>
        ...
        <xsl:variable name="bulk-day-begin"
          select="
                        ...
                        then
                            ...
                            ''"/>
        <xsl:variable name="bulk-year-end" select="
        ...
        <xsl:variable name="bulk-month-end"
          select="
                        ...
                        then
                            ...
                            ''"/>
        <xsl:variable name="bulk-day-end" select="
        ...
        <unitdate type="bulk">
          <xsl:attribute name="normal">
            <xsl:choose>
              <xsl:when test="
                ...
                <xsl:value-of select="concat($bulk-year-begin, $bulk-month-begin, $bulk-day-begin)"/>
              </xsl:when>
              <xsl:otherwise>
                <xsl:value-of select="concat($bulk-year-begin, $bulk-month-begin, $bulk-day-begin, '/', $bulk-year-end, $bulk-month-end, $bulk-day-end)"/>
              </xsl:otherwise>
            </xsl:choose>
          </xsl:attribute>
          ...
          <xsl:choose>
            <xsl:when test="$bulk-year-end != ''">
              <xsl:value-of select="concat($bulk-year-begin,'-',$bulk-year-end)"/>
            </xsl:when>
            <xsl:otherwise>
              <xsl:value-of select="$bulk-year-begin"/>
            </xsl:otherwise>
          </xsl:choose>
        </unitdate>
      </xsl:when>
      ...
      <xsl:when test="$column-number eq 28">
        <physdesc>
          ...
          <xsl:if test="following-sibling::ss:Cell[ss:NamedCell/@ss:Name = 'generic_extent'][ss:Data[normalize-space()]]">
            <extent>
              <xsl:apply-templates select="following-sibling::ss:Cell[ss:NamedCell/@ss:Name = 'generic_extent']"/>
            </extent>
          </xsl:if>
        </physdesc>
      </xsl:when>
